...
second_score = "%.2f%%" % (index.iloc[1, :]['周涨跌幅'] * 100)
third_name = index.iloc[-2, :]['产品名称']
third_score = "%.2f%%" % (index.iloc[-2, :]['周涨跌幅'] * 100)
...
second_name = index.iloc[1, :]['产品名称']
second_score = "%.2f%%" % (index.iloc[1, :]['上周超额'] * 100)
third_name = index.iloc[-2, :]['产品名称']
third_score = "%.2f%%" % (index.iloc[-2, :]['上周超额'] * 100)
last_name = index.iloc[-1, :]['产品名称']
last_score = "%.2f%%" % (index.iloc[-1, :]['上周超额'] * 100)
up = document.add_paragraph('指数增强策略中，')
if n1 == 0:
    up.add_run('本周所有在架产品超额为负。其中，')
    up.add_run(str(last_name))
    up.add_run('领跌，录得')
    up.add_run(str(last_score))
    up.add_run('的负超额；')
...
up.add_run('。')
# 输出word
document.save('output/业绩周报告.docx')

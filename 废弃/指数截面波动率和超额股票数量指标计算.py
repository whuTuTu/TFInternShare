...
# IPO时间
IPOdate = THS_BD(allStock1, 'ths_ipo_date_stock', '').data

for i in range(date_len):
    # 全市场
    selected_rows = IPOdate[IPOdate['ths_ipo_date_stock'] == date[i]]
    result_list = selected_rows['thscode'].tolist()
    DF5000 = allRatioDF1[~allRatioDF1['thscode'].isin(result_list)].dropna(subset=[column_names[i + 1]])[

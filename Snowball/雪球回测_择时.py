# -*- coding: utf-8 -*-
"""
Created on Tue Jun 27 10:00:19 2023
...
df = pd.read_excel(r'input/雪球输入.xlsx',sheet_name='估值分位择时-1000')  # 从csv文件读取数据，您需要根据您的数据源进行修改
...
def snowball_return(start_date):
    ...
    for date, close in sub_df.items():
        ...
        if holding_days < 40: #两个月锁定期
            continue #如果在锁定期内，跳过循环剩余部分，进行下一次循环
    ...
    if knocked_out:  # 如果发生了敲出事件，提前终止合约，获得年化票息收益
        ...
        if holding_days > 480: #两年到期
            return_rate = coupon_rate
        else: return_rate = None #未到期
    else:  # 如果发生了敲入事件，且到期未敲出到期终止合约，承担标的下跌造成的损失
        if holding_days > 480: #到期未敲出，但是标的价格大于初始价格，则收益率为0
            if close >= initial_price: 
                return_rate = 0
            else: return_rate = (close - initial_price) / initial_price #到期收盘价小于初始价格，承担亏损
        else:
            return_rate = None

    # 返回合约的状态和收益
    return knocked_out, knocked_in, out_date, in_date, holding_days, return_rate,start_date

########backtest#########
# 定义一个空的列表，用于存储每个合约的状态和收益
results = []

# 遍历每个交易日，假设在pe分位在20%以下或80%以上的时候买入雪球合约，并调用上面定义的函数模拟其收益情况
for date, row in df[df['signal'] == 1].iterrows():
...
results_df = pd.DataFrame(results,
                          columns=['knocked_out', 'knocked_in', 'out_date', 'in_date', 'holding_days', 'return_rate','start_date'])
results_df.to_excel(r'output/雪球择时输出.xlsx')
...
plt.xlabel('Strategy Return Rate')
